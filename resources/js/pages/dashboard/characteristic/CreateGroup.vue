...
const injected = inject<CreateGroup>('creategroup')

if (!injected) {
    throw new Error('Injection "creategroup" not found')
}

const { form, onCreate, onUpdate } = injected
</script>
    	...
    	<q-item>
			<q-item-section class="justify-around col-6">
				...
				<input-string v-model="form.title" label="Наименование" class="q-field--with-bottom"
                    :error_message="form.errors.title" :error="!!form.errors.title"
                />
				<input-string v-model="form.description" label="Описание" class="q-field--with-bottom q-pb-none"
                    :error_message="form.errors.description" :error="!!form.errors.description"
                />
			</q-item-section>
